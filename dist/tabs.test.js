...
describe('Tabs', function () {
  ...
  it('接收 selected', function (done) {
    ...
    vm.$nextTick(function () {
      var element = vm.$el.querySelector('.tabs-item[data-name="sport"]');
      expect(element.classList.contains('active')).to.be.eq(true);
      done();
    });
  });
  it('接收 direction', function (done) {
    var div = document.createElement('div');
    document.body.appendChild(div);
    ...
    vm.$nextTick(function () {
      var tabs = vm.$el.querySelector('.tabs');
      expect(tabs.classList.contains('vertical')).to.be.eq(true);
      var head = vm.$el.querySelector('.tabs-head');
      expect(head.classList.contains('vertical')).to.be.eq(true);
      var item = vm.$el.querySelector('.tabs-item');
      expect(item.classList.contains('vertical')).to.be.eq(true);
      var pane = vm.$el.querySelector('.tabs-pane');
      expect(pane.classList.contains('vertical')).to.be.eq(true);
      done();
    });
  });
});
},{"vue":"ApMz","../src/tabs.vue":"n6Xd","../src/tabs-head.vue":"Q07j","../src/tabs-item.vue":"albi","../src/tabs-body.vue":"Ydtc","../src/tabs-pane.vue":"vREI"}]},{},["eztQ"], null)
